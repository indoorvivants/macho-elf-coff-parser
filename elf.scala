...
object ELF {
  ...
  def parse(ds: BinaryFile): ELF = {
    ...
    readSectionHeaders(sectionsHeaderEntries)
  ...
  def readSectionHeaders(
    ...
    for (i <- 0 until entries) {
      ...
      if(sectionType == 0x03) pprint.log(s"$i ${offsetInFileImage.toHexString}")
    }
  ...
  def readVariableSize()(implicit ds: BinaryFile, bits: Bits, endi: Endianness) =
    bits match {
